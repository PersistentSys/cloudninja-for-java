 ...
 * tenant DB size calculation task.
 *
 */
@Component
public class TenantDBSizeScheduler {
	
	private static final Logger LOGGER = Logger.getLogger(TenantDBSizeScheduler.class);
	
	@Autowired
	private TenantDBSizeGenerator tenantDBSizeGenerator;
	
	@Autowired
	private TenantDBSizeProcessor tenantDBSizeProcessor;
	
	@Autowired
	private StorageUtility storageUtility;

	/**
	 * This is a generator task which will listen to taskscheduler-tenantdbsize 
	 * for a message, if message exists then call execute() and delete the message 
	 * from taskscheduler-tenantdbsize 
	 */
	...
	public void generator() {
		...
		try {
			message = storageUtility.getMessage("taskscheduler-" 
					+ SchedulerSettings.DatabaseSizeTask.toLowerCase());
			if (message != null) {
				LOGGER.debug("SCH-generator : msg is not null");
				if(tenantDBSizeGenerator.execute()) {
					storageUtility.deleteMessage("taskscheduler-" 
							+ SchedulerSettings.DatabaseSizeTask.toLowerCase(), message);
	...
	public void processor() {
		...
		if (tenantDBSizeProcessor.execute()) {
			try {
				LOGGER.debug("SCH-processor : execute");
				ActivityWorkQueue workQueue = tenantDBSizeProcessor.getWorkQueue();
				workQueue.complete();
